...
btc <- get_stock_zoo('BTC-USD')
link <- get_stock_zoo('LINK-USD')
summary(btc)
```

## Graphing the Time Series

Figure 1 shows the time series for Bitcoin over the past 5 years. It appears to be a multiplicative, non-stationary time series with an exponential positive trend that has exploded most recently in 2021.

```{r data2, include=T,echo=T,warning=FALSE}
autoplot(data2)

cowplot::plot_grid(
  autoplot(btc)+ggtitle("bitcoin"),autoplot(link)
)
```
