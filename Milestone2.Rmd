...
end_date = '2021-09-30'
tickers = c('SPY','AMD','NVDA','BTC-USD','ETH-USD','LINK-USD')
...
if(file.exists('tickers.csv')){
  ...
  stocks = tq_get(tickers,
                  from=start_date,
                  to=end_date
                  )[,c('symbol','date','volume','adjusted')]
  stocks$date = ymd(stocks$date)
  write.csv(stocks,'tickers.csv')
}

# A function that will return only one stock as zoo object
get_stock_zoo <- function(ticker){
    data = zoo(
      stocks[stocks$symbol==ticker,c('adjusted')],
      order.by = stocks[stocks$symbol==ticker,c('date')]
      )
    return(data)
}
```
